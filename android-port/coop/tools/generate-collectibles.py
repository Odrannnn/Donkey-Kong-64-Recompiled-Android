...
for m in range(221):
    ...
    for i in range(actor_count):
        ...
        if typ in (0x5B, 0x6F, 0x70, 0x71, 0x72, 0x8B):
            ...
            actors.append((flag, m, level(m), kong, 0 if typ == 0x8B else 10))
assert actor_ordinal == 121 and len(actors) == 120
assert sum(a[4] == 0 for a in actors) == 16
...
lines += [f'    {{0x{flag:03X}, {m}, {l}, {k}}},' for flag, (m, l, k) in sorted(gbs.items())]
lines += ['};', 'typedef struct { unsigned char map, kong, amount; } CoopPickup;',
...
for l, rows in enumerate(pickups):
    ...
    lines += [f'    {{{m}, {k}, {amount}}}, // object {obj}' for k, amount, m, obj in rows]
lines += ['};', 'typedef struct { unsigned short flag; unsigned char map, level, kong, amount; } CoopActorPickup;',
          '// amount 10 = balloon; amount 0 = rainbow coin (+5 to every Kong).',
          'static const CoopActorPickup coop_actor_pickups[120] = {']
lines += [f'    {{0x{f:03X}, {m}, {l}, {k}, {a}}},' for f, m, l, k, a in actors]
lines += ['};', '#endif', '']
output = '\n'.join(lines)
