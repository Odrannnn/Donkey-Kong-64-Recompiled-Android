...
bool failed = false;
struct ExtraInput { dkcoop::ProgressInput gate; CoopCombatFrame combat; CoopItemInput items; CoopWorldInput world; };
struct ExtraResult { dkcoop::ProgressResult gate; CoopCombatResult combat; CoopItemResult items; CoopWorldResult world; };
...
COOP_EXPORT void dk64_coop_tick_v43(uint8_t* rdram, recomp_context* ctx) {
    ...
    static_assert(sizeof(ExtraInput) == 2612 && sizeof(ExtraResult) == 3300);
    ...
    try {
        ...
        // bridge and wire slots remain reserved and canonical zero in v43.
        ...
        ExtraResult result{{}, session.combat(now), session.items(now), session.world(now)};
