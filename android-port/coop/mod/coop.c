...
_Static_assert(sizeof(CoopExtraInput) == 2868 && sizeof(CoopExtraResult) == 3476, "v61 bridge ABI");
...
RECOMP_IMPORT(".", u32 dk64_coop_recovery_configure_v61(const char* save_path, u32 profile, u32 save_kind, u32 room));
RECOMP_IMPORT(".", u32 dk64_coop_recovery_status_v61(void));
RECOMP_IMPORT(".", u32 dk64_coop_recovery_promote_v61(void));
RECOMP_IMPORT(".", u32 dk64_coop_tick_v61(const u32* local, u32* remote, const CoopExtraInput* input, CoopExtraResult* result));
...
static void coop_select_save(void) {
    ...
    recovery_storage_status = dk64_coop_recovery_configure_v61((const char*)save_path,
...
static u32 coop_promote_guest(void) {
    ...
    if (!dk64_coop_recovery_promote_v61()) {
        dk64_coop_stop();
        status = coop_start_network(ROLE_JOIN);
        coop_host_recovery_complete(&host_recovery, 0);
        return 0;
...
RECOMP_CALLBACK("*", dk64recomp_every_frame) void coop_frame(void) {
    ...
    status = dk64_coop_tick_v61(local_state, remote_state, &extra, &extra_result);
    ...
    recovery_storage_status = dk64_coop_recovery_status_v61();
