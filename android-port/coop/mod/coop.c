...
_Static_assert(sizeof(CoopExtraInput) == 2868 && sizeof(CoopExtraResult) == 3476, "v77 bridge ABI");
...
_Static_assert(COOP_MAP_JAPES_DILLO == 8 && COOP_MAP_FUNGI_DOGADON == 83
    ...
    && COOP_MAP_K_ROOL_SHOE == 214 && COOP_MAP_FUNGI_SPIDER == 60
...
_Static_assert(MAP_CAVES_ICE_CASTLE == 98 && ACTOR_TOMATO_ICE == 164
    ...
    && COOP_TRANSIENT_TOMATO_CLOCK + 1 == COOP_TRANSIENT_MINIGAME_SUCCESS,
    "Pinned Ice Tomato same-area contract");
_Static_assert(MAP_KREMLING_KOSH_VERY_EASY == 10 && MAP_KREMLING_KOSH_EASY == 115
    && MAP_KREMLING_KOSH_NORMAL == 116 && MAP_KREMLING_KOSH_HARD == 117
    && ACTOR_MINIGAME_CONTROLLER == 256,
    "Pinned Kremling Kosh same-area contract");
_Static_assert(MAP_MINECART_MAYHEM_EASY == 77 && MAP_MINECART_MAYHEM_NORMAL == 129
    && MAP_MINECART_MAYHEM_HARD == 130 && MAP_JAPES == 7 && MAP_FUNGI_DK_BARN == 59
    && MAP_CASTLE_BALLROOM == 88 && ACTOR_MINECART_BONUS == 87
    && COOP_TRANSIENT_MINIGAME_SUCCESS + 1 == COOP_TRANSIENT_MINECART_SUCCESS,
    "Pinned Minecart Mayhem same-area contract");
_Static_assert(MAP_FUNGI == 48 && ACTOR_RABBIT_RACE == 252
    && PERMFLAG_PROGRESS_RABBIT_RACE_1_COMPLETE == 0xF8
    && PERMFLAG_ITEM_GB_FUNGI_RABBIT_RACE == 0xF9
    && COOP_TRANSIENT_MINECART_SUCCESS + 1 == COOP_TRANSIENT_RABBIT_SUCCESS,
    "Pinned Fungi Rabbit Race same-area contract");
_Static_assert(MAP_BATTY_BARREL_BANDIT_EASY == 32
    && MAP_BATTY_BARREL_BANDIT_EASY_2 == 121
    && MAP_BATTY_BARREL_BANDIT_NORMAL == 122
    && MAP_BATTY_BARREL_BANDIT_HARD == 123
    && ACTOR_BANDIT_HANDLE == 218 && ACTOR_BANDIT_SLOT == 219
    && ACTOR_TIMER == 176
    && COOP_TRANSIENT_RABBIT_SUCCESS + 1 == COOP_TRANSIENT_BATTY_SUCCESS
    && ACTOR_OWL == 250
    && COOP_TRANSIENT_BATTY_SUCCESS + 1 == COOP_TRANSIENT_OWL_SUCCESS
    && COOP_TRANSIENT_KIND_COUNT == COOP_TRANSIENT_OWL_SUCCESS,
    "Pinned Batty Barrel Bandit same-area contract");
...
RECOMP_IMPORT(".", u32 dk64_coop_recovery_configure_v77(const char* save_path, u32 profile, u32 save_kind, u32 room));
RECOMP_IMPORT(".", u32 dk64_coop_recovery_status_v77(void));
RECOMP_IMPORT(".", u32 dk64_coop_recovery_promote_v77(void));
RECOMP_IMPORT(".", u32 dk64_coop_tick_v77(const u32* local, u32* remote, const CoopExtraInput* input, CoopExtraResult* result));
...
// Isles object 0x31 is the trombone pad which reveals Diddy's high rocket
// barrel. Its stock completion sends actor-spawner notification (9, 0), leaves
// the script in state 6 and selects pad mode 2. A remote permanent flag may
// perform those exact final operations only while the local sequence is idle;
// states 3-6 already own their cutscene/reveal path and must continue untouched.
...
static unsigned coop_live_world_isles_trombone_refresh(void) {
    for (unsigned slot = 0; slot < 600; ++slot) {
        if ((unsigned short)D_global_asm_807F6240[slot] != 0x31) continue;
        s16 prop = func_global_asm_80659470(D_global_asm_807F6240[slot]);
        if (prop < 0 || !D_global_asm_807F6000 || !D_global_asm_807F6000[prop].unk7C) return 0;
        Prop_ScriptData* script = D_global_asm_807F6000[prop].unk7C;
        unsigned raw = (unsigned short)script->unk48[0];
        if (raw > 6) return 0;
        if (raw >= 3) return 1;
        func_global_asm_80641874(9, 0);
        script->unk48[0] = 6;
        script->unk54 = 2;
        return 1;
    }
...
static void coop_select_save(void) {
    ...
    recovery_storage_status = dk64_coop_recovery_configure_v77((const char*)save_path,
...
static u32 coop_promote_guest(void) {
    ...
    if (!dk64_coop_recovery_promote_v77()) {
        dk64_coop_stop();
        status = coop_start_network(ROLE_JOIN);
        coop_host_recovery_complete(&host_recovery, 0);
        return 0;
...
RECOMP_CALLBACK("*", dk64recomp_every_frame) void coop_frame(void) {
    ...
    status = dk64_coop_tick_v77(local_state, remote_state, &extra, &extra_result);
    ...
    recovery_storage_status = dk64_coop_recovery_status_v77();
