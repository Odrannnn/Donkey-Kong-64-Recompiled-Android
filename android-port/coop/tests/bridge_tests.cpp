...
extern "C" void dk64_coop_tick_v42(uint8_t*, recomp_context*);
...
int main() {
    ...
    for (unsigned i = 0; i < 1000; i++) {
        ...
        dk64_coop_tick_v42(rdram.data(), &ctx);
    ...
    rdram[4096 + sizeof(dkcoop::ProgressResult) + sizeof(CoopCombatResult) + sizeof(CoopItemResult) + sizeof(CoopWorldResult)] = 0xA5;
    ...
    CHECK(rdram[4096 + sizeof(dkcoop::ProgressResult) + sizeof(CoopCombatResult) + sizeof(CoopItemResult) + sizeof(CoopWorldResult)] == 0xA5);
    ...
    h.map = g.map = 8;
    local = dkcoop::state_to_words(g); std::memcpy(rdram.data() + 512, local.data(), dkcoop::state_words * 4);
    hc = {}; hc.enabled = 1; hc.file = 1; hc.boss = {COOP_BOSS_ARMY_DILLO, 700, 0, 1};
    gc = {}; gc.enabled = 1; gc.file = 2; gc.boss = {COOP_BOSS_ARMY_DILLO, 800, 0, 0};
    std::memcpy(rdram.data() + 1024 + sizeof(dkcoop::ProgressInput), &gc, sizeof(gc));
    ...
    CHECK(boss_exchanged && rdram[4096 + sizeof(dkcoop::ProgressResult) + sizeof(CoopCombatResult)
        + sizeof(CoopItemResult) + sizeof(CoopWorldResult)] == 0xA5);
    h.map = g.map = 83;
    local = dkcoop::state_to_words(g); std::memcpy(rdram.data() + 512, local.data(), dkcoop::state_words * 4);
    hc = {}; hc.enabled = 1; hc.file = 1; hc.boss = {COOP_BOSS_DOGADON, 900, 0, 4};
    gc = {}; gc.enabled = 1; gc.file = 2; gc.boss = {COOP_BOSS_DOGADON, 901, 0, 3};
    std::memcpy(rdram.data() + 1024 + sizeof(dkcoop::ProgressInput), &gc, sizeof(gc));
    ...
    h.map = g.map = 7;
    local = dkcoop::state_to_words(g); std::memcpy(rdram.data() + 512, local.data(), dkcoop::state_words * 4);
    // v42 retires the standalone Japes gate. Nonzero data in its reserved
    ...
    CHECK(items_exchanged && rdram[item_out + sizeof(CoopItemResult) + sizeof(CoopWorldResult)] == 0xA5);
    for (uint32_t bad : {0u, 0x7fffffffu, 0x80000201u, 0x9ffffffcu, 0xffffffffu}) {
        ctx.r4 = bad; ctx.r5 = 0x80000300u; dk64_coop_tick_v42(rdram.data(), &ctx);
    ...
    for (uint32_t bad : {0u, 0x80000401u, 0x9ffffc78u, 0x9ffffd00u, 0x9ffffe00u, 0x9ffffffcu, 0xffffffffu}) {
        ...
        dk64_coop_tick_v42(rdram.data(), &ctx); CHECK(ctx.r2 == uint32_t(dkcoop::Status::error));
        ctx.r6 = 0x80000400u; ctx.r7 = bad;
        dk64_coop_tick_v42(rdram.data(), &ctx); CHECK(ctx.r2 == uint32_t(dkcoop::Status::error));
    }
    ctx.r6 = 0x80000400u; ctx.r7 = 0x9ffffb00u; // 1280 bytes remain: full v42 result cannot fit.
    dk64_coop_tick_v42(rdram.data(), &ctx); CHECK(ctx.r2 == uint32_t(dkcoop::Status::error));
    ctx.r7 = 0x9ffffa48u; // 1464 bytes remain: older results fit, full v42 must fail.
    dk64_coop_tick_v42(rdram.data(), &ctx); CHECK(ctx.r2 == uint32_t(dkcoop::Status::error));
    ctx.r6 = 0xA0000000u - 2600u; ctx.r7 = 0x80001000u; // Four bytes short of v42 input.
    dk64_coop_tick_v42(rdram.data(), &ctx); CHECK(ctx.r2 == uint32_t(dkcoop::Status::error));
    ctx.r6 = 0x80000400u; ctx.r7 = 0xA0000000u - 3296u; // Four bytes short of v42 result.
    dk64_coop_tick_v42(rdram.data(), &ctx); CHECK(ctx.r2 == uint32_t(dkcoop::Status::error));
    ...
    ctx.r4 = 1; ctx.r5 = 0; ctx.r6 = 6465; ctx.r7 = 123456;
    ...
    dk64_coop_stop(rdram.data(), &ctx); CHECK(ctx.r2 == 0);
    std::puts("PASS: v42 native ABI, host IPv4 reporting, retired gate isolation, coordinated transitions, Army Dillo/Dogadon phases, sign-extended pointers, pose/shot/enemy movement, item grants/requests and invalid address guards");
}
