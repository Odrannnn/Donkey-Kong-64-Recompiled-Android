...
extern "C" void dk64_coop_recovery_configure_v67(uint8_t*, recomp_context*);
extern "C" void dk64_coop_recovery_status_v67(uint8_t*, recomp_context*);
extern "C" void dk64_coop_tick_v67(uint8_t*, recomp_context*);
...
int main() {
    ...
    dk64_coop_recovery_configure_v67(rdram.data(), &ctx);
    CHECK(ctx.r2 == COOP_RECOVERY_STORAGE_CONFIGURED);
    dk64_coop_recovery_status_v67(rdram.data(), &ctx);
    CHECK(ctx.r2 == COOP_RECOVERY_STORAGE_CONFIGURED);
    ...
    for (unsigned i = 0; i < 1000; i++) {
        ...
        dk64_coop_tick_v67(rdram.data(), &ctx);
    ...
    for (uint32_t bad : {0u, 0x7fffffffu, 0x80000201u, 0x9ffffffcu, 0xffffffffu}) {
        ctx.r4 = bad; ctx.r5 = 0x80000300u; dk64_coop_tick_v67(rdram.data(), &ctx);
    ...
    for (uint32_t bad : {0u, 0x80000401u, 0x9ffffc78u, 0x9ffffd00u, 0x9ffffe00u, 0x9ffffffcu, 0xffffffffu}) {
        ...
        dk64_coop_tick_v67(rdram.data(), &ctx); CHECK(ctx.r2 == uint32_t(dkcoop::Status::error));
        ctx.r6 = 0x80000400u; ctx.r7 = bad;
        dk64_coop_tick_v67(rdram.data(), &ctx); CHECK(ctx.r2 == uint32_t(dkcoop::Status::error));
    }
    ctx.r6 = 0x80000400u; ctx.r7 = 0x9ffffb00u; // 1280 bytes remain: full v67 result cannot fit.
    dk64_coop_tick_v67(rdram.data(), &ctx); CHECK(ctx.r2 == uint32_t(dkcoop::Status::error));
    ctx.r7 = 0x9ffffa48u; // 1464 bytes remain: older results fit, full v67 must fail.
    dk64_coop_tick_v67(rdram.data(), &ctx); CHECK(ctx.r2 == uint32_t(dkcoop::Status::error));
    ctx.r6 = 0xA0000000u - 2864u; ctx.r7 = 0x80001000u; // Four bytes short of v67 input.
    dk64_coop_tick_v67(rdram.data(), &ctx); CHECK(ctx.r2 == uint32_t(dkcoop::Status::error));
    ctx.r6 = 0x80000400u; ctx.r7 = 0xA0000000u - 3472u; // Four bytes short of v67 result.
    dk64_coop_tick_v67(rdram.data(), &ctx); CHECK(ctx.r2 == uint32_t(dkcoop::Status::error));
    ...
    std::puts("PASS: v67 native ABI, host IPv4 reporting, transient span isolation, coordinated transitions, Army Dillo/Dogadon phases, sign-extended pointers, pose/shot/enemy movement, item grants/requests and invalid address guards");
}
