...
static void world_refresh_checks() {
    ...
    // Aztec lobby replays both saved-complete initializers atomically, leaving
    // the combat/reward sequence local while applying its permanent platform.
    reset_engine(); current_map = 173;
    D_global_asm_807F6240[1] = 0x04; D_global_asm_807F6240[2] = 0x11;
    CHECK(coop_live_world_refresh(0x190) && live_calls == 2
        && live_slot == 2 && live_state == 0);
    reset_engine(); current_map = 173; D_global_asm_807F6240[1] = 0x11;
    CHECK(!coop_live_world_refresh(0x190) && live_calls == 0);

    // Fungi lobby applies only the saved-complete gun-order controller and
    // panel branches, with no synthesized hit sequence or reward path.
    reset_engine(); current_map = 178;
    D_global_asm_807F6240[1] = 0x09; D_global_asm_807F6240[2] = 0x04;
    CHECK(coop_live_world_refresh(0x195) && live_calls == 2
        && live_slot == 2 && live_state == 0);
    ...
    // The five Caves pads and spent reveal controller update atomically.
    reset_engine(); current_map = 72;
    const unsigned short caves_pads[] = {0x00, 0x01, 0x02, 0x03, 0x04, 0x38};
    for (unsigned i = 0; i < 6; ++i) D_global_asm_807F6240[10 + i] = caves_pads[i];
    CHECK(coop_live_world_refresh(0x128) && live_calls == 6
        && live_slot == 15 && live_state == 0);
    reset_engine(); current_map = 72;
    for (unsigned i = 0; i < 5; ++i) D_global_asm_807F6240[10 + i] = caves_pads[i];
    CHECK(!coop_live_world_refresh(0x128) && live_calls == 0);

    // The saved Caves encounter clear disables its delayed spawn controller.
    reset_engine(); current_map = 72; D_global_asm_807F6240[4] = 0x34;
    CHECK(coop_live_world_refresh(0x12C) && live_calls == 1
        && live_slot == 4 && live_state == 0);

    // Castle ballroom enters its saved-clear barrel-reveal path live.
    ...
    // Helm's coin door applies its saved-open initializer without spending.
    reset_engine(); current_map = 17; D_global_asm_807F6240[6] = 0x03;
    CHECK(coop_live_world_refresh(0x303) && live_calls == 1
        && live_slot == 6 && live_state == 0);

    // Helm's crown door likewise applies only its saved-open initializer.
    reset_engine(); current_map = 17; D_global_asm_807F6240[8] = 0x04;
    CHECK(coop_live_world_refresh(0x304) && live_calls == 1
        && live_slot == 8 && live_state == 0);

    // Helm shutdown preflights all 21 flag consumers and the five medal
    // barriers reached by their vanilla linked calls, then wakes consumers only.
    reset_engine(); current_map = 17;
    const unsigned short helm_shutdown[] = {
        0x005, 0x02D, 0x02E, 0x02F, 0x030,
        0x020, 0x00E, 0x008, 0x026, 0x011, 0x009, 0x024, 0x00A, 0x00F,
        0x01E, 0x00B, 0x010, 0x022, 0x00D, 0x00C, 0x1F6,
        0x01F, 0x021, 0x023, 0x025, 0x027,
    };
    for (unsigned i = 0; i < 26; ++i)
        D_global_asm_807F6240[30 + i] = (short)helm_shutdown[i];
    CHECK(coop_live_world_refresh(0x302) && live_calls == 21
        && live_slot == 50 && live_state == 0);
    reset_engine(); current_map = 17;
    ...
    // Aztec's totem activation replays all seven saved-complete initializers
    // only when the full temple unit is loaded.
    reset_engine(); current_map = 38;
    const unsigned short aztec_totem[] = {0x28, 0x27, 0x10, 0x11, 0x12, 0x13, 0x14};
    for (unsigned i = 0; i < 7; ++i)
        D_global_asm_807F6240[70 + i] = (short)aztec_totem[i];
    CHECK(coop_live_world_refresh(0x037) && live_calls == 7
        && live_slot == 76 && live_state == 0);
    reset_engine(); current_map = 38;
    for (unsigned i = 0; i < 6; ++i)
        D_global_asm_807F6240[70 + i] = (short)aztec_totem[i];
    CHECK(!coop_live_world_refresh(0x037) && live_calls == 0);
